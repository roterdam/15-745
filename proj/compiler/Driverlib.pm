...
require Exporter;
our @ISA = qw(Exporter);
our @EXPORT = qw(&report_result &system_with_timeout &printd &printq &progress &read_file &read_last_line $DEBUG $QUIET);
...
###
# read_last_line($file)
# returns last line of file as a single string
#
sub read_last_line {
  my $file = shift;
  my $last_line;
  ...
  while (my $line = readline $fh) {
    if ( $line !~ /^$/ ) { $last_line = $line } # line is non-empty
  }
  return $last_line;
}
